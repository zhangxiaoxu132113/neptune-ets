package com.water.neptune.ets.common.dao.plus.mybatis.executor;
...
public class PageExecutor implements Executor {
    ...
    private static Pattern pOrderBy = Pattern.compile("ORDER\\s*by[\\w|\\W|\\s|\\S]*", Pattern.CASE_INSENSITIVE);
    ...
    private <E> List<E> pageResolver(List<E> rows, MappedStatement ms, Object parameter, RowBounds rowBounds) {
        ...
        PageParam pageParam = PageParam.getPageParam(parameter);
        if (msid.matches(pattern)) {
            int count = getCount(ms, parameter);
            return new PageResult<E>(pageParam.getDefaultPage(), pageParam.getDefaultPageSize(), count, rows);
        }
        return rows;
    }
    ...
    private String removeOrders(String sql) {
        Matcher m = pOrderBy.matcher(sql);
    ...
    @Override
    public <E> Cursor<E> queryCursor(MappedStatement mappedStatement, Object o, RowBounds rowBounds)
            throws SQLException {
        return null;
    }
